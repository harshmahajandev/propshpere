-- Migration: populate_financial_records_comprehensive_v3
-- Created at: 1759239855
...
SELECT 
  ...
  'bank_transfer',
  'MKT-' || TO_CHAR(NOW() - INTERVAL '15 days', 'YYYYMM') || '-' || LPAD(generate_series::text, 3, '0'),
...
SELECT 
  'commission',
  'sales_commission',
  (8000 + RANDOM() * 12000)::NUMERIC,  -- BHD 8K-20K sales commissions
  'BHD',
  (NOW() - INTERVAL '10 days')::DATE,
  NULL,
  NULL,
  CASE generate_series % 3
    WHEN 0 THEN 'Sales commission for Al Naseem deals'
    WHEN 1 THEN 'Team bonus for exceeding monthly targets'
    ELSE 'Agent commission for premium villa sales'
  END,
  'bank_transfer',
  'COM-' || TO_CHAR(NOW() - INTERVAL '10 days', 'YYYYMM') || '-' || LPAD(generate_series::text, 3, '0'),
  NOW() - INTERVAL '10 days'
FROM generate_series(1, 5)

UNION ALL

-- Recent revenue from reservations
SELECT 
  'revenue',
  'reservation',
